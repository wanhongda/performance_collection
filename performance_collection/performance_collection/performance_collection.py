# -*- encoding: utf-8 -*-
"""
@File    :   performance_collection
@Author  :   wanhongda
@Modify Time      @Version    @Desciption
------------      --------    -----------
2023/4/24 7:20    1.0         None
"""
import datetime
import re
import time
...
class APM(object):
    ...
    def collectGpu(self):
        ...
        return result
# 获取部分数据奥
    def collectAll_new(self):
        _cpu = CPU(self.pkgName, self.deviceId, self.platform, pid=self.pid)
        _mem = MEM(self.pkgName, self.deviceId, self.platform, pid=self.pid)
        _battery = Battery(self.deviceId, self.platform)
        _flow = Flow(self.pkgName, self.deviceId, self.platform, pid=self.pid)
        _fps = FPS(self.pkgName, self.deviceId, self.platform)

        appCpuRate, systemCpuRate = _cpu.getCpuRate(noLog=self.noLog)
        totalPass, nativePass, dalvikPass = _mem.getProcessMem(noLog=self.noLog)
        level, temperature = _battery.getAndroidBattery(noLog=self.noLog)
        sendNum, recNum = _flow.getNetWorkData(noLog=self.noLog)
        fps, jank = _fps.getFPS(noLog=self.noLog)
        result = {'appCpuRate': appCpuRate, 'systemCpuRate': systemCpuRate, 'totalPass': totalPass,
                  'nativePass': nativePass, 'dalvikPass': dalvikPass, 'battery_level': level, 'temperature': temperature,
                  'flow_upFlow': sendNum, 'flow_downFlow': recNum, 'fps': fps, 'jank': jank}
